...
@<strong>{「アーキテクチャを示す1文字 + ツールの種類1文字」}という規則で
命名されています. アーキテクチャを示す1文字には主に数字(たまにアルファベット)が
用いられます@<fn>{arch_O}. それぞれの文字が対応するアーキテクチャを以下に
列挙します. 
...
@<strong>{消滅} と末尾に付記したアーキテクチャは, 各種ドキュメントにその記録が
あるものの9front や Plan 9 4th Edition ではソースコード (src/cmd/ の下) から
コンパイラ等が消えてしまっているものです. 
...
先に述べたとおりこれらのツールは
@<strong>{「アーキテクチャを示す1文字 + ツールの種類1文字」}の名前が
付けられています. ツールの種類1文字としてコンパイラは @<strong>{c}, 
アセンブラは @<strong>{a}, そして ローダは @<strong>{l} を用います. 
...
ローダという名前は, 各オブジェクトファイル内のコードを実行ファイル内へと
ロード, 適切に配置していくという処理を元に付けられています. 


ローダはこのように実行形式を形作るにあたり以下の様なことも行っています. 
