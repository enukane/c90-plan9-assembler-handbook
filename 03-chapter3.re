...
この章では, 前章で解説した一般的なPlan 9 アセンブラのお作法から
...
これを使いません. ビット幅については第2章で述べたロード・ストアを担当する MOV 
疑似命令のサフィクスを持って判断されます. 
...
がこれらを使うことはできません. 8a では 2a つまり MC68020 向けの分岐命令i
...
あります. これらは絶対番地へのジャンプ等に用いられます. 
@<list>{8a_JMP_asterisk}は, カーネルを起動するにあたり絶対番地に飛ぶために
これを用いています. このような起動時の処理などでのみ用いられます. 
...
8a でも REP, REPN を用いることができますが, プレフィクス的に
用いることはできません. 
